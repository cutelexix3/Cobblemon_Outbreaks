    ...
    public void tick() {
        ...
        if (!level.isClientSide) {
            ...
            if (getTickData().getTickCount() % 100 == 0 && getWaveData().getWave() < getPortal().getSpeciesData().getWaveData().getWaves() && !containsPokemon) {
                spawnWave();
                if(!level.isClientSide) {
                    CobblemonOutbreaksEvent.Events.WAVE_END.invoker().onWaveEnd((ServerLevel) level, this);
                }
...
//
    ...
    public void completeOutbreak(boolean rewards) {
        if (level instanceof ServerLevel serverLevel) {
            ...
            CobblemonOutbreaksEvent.Events.PORTAL_CLOSE.invoker().onPortalClose(serverLevel, this);
        }
    }
